"""
Telegram gateway panel — MarkdownV2 copy + inline keyboards only.

No Flask, no web server, no blocking file IO. Used by ``gateway.telegram_bot``.
"""
...
try:
    ...
    from telegram.helpers import escape_markdown as _tg_gw_escape_markdown
except ImportError:  # pragma: no cover
    InlineKeyboardButton = None  # type: ignore[misc, assignment]
    InlineKeyboardMarkup = None  # type: ignore[misc, assignment]

    def _tg_gw_escape_markdown(text: str, version: int = 2) -> str:
        if version != 2:
            return text
        s = str(text)
        for ch in r"_*[]()~`>#+-=|{}.!":
...
def tg_gw_escape_v2(text: str) -> str:
    """Escape dynamic segments for Telegram ``parse_mode='MarkdownV2'``."""
    if not text:
        return text
    return _tg_gw_escape_markdown(str(text), version=2)


def tg_gw_render_callback_pending_text() -> str:
    """网关按钮秒回：零延迟占位（不含模式横幅，由后续任务覆盖）。"""
    return tg_gw_escape_v2("⏳ 脑机接口已连接，正在请求云端算力…")
...
def _tg_gw_mode_banner_v2(mode: str) -> str:
    label = tg_gw_escape_v2(tg_gw_mode_label(mode))
    return f"*当前交易模式：* {label}"


def tg_gw_build_main_keyboard(mode: str):
    """主页状态机：模拟盘 / 实盘切换（持久化见 SessionStore）+ 持仓 / 策略。"""
...
def tg_gw_render_home_text(mode: str) -> str:
    banner = _tg_gw_mode_banner_v2(mode)
    m = (mode or "paper").lower()
    hint = (
...
def tg_gw_render_positions_text(mode: str, snap: dict | None, *, refreshing: bool = False) -> str:
    if not snap:
        return (
    ...
    body = _tg_gw_format_snapshot_v2(mode, snap)
    if refreshing:
        body = body + "\n\n_" + tg_gw_escape_v2("后台同步中…") + "_"
    return body
