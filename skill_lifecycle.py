...
def classify_task(message: str) -> dict:
    ...
    Returns: {"type": str, "confidence": float, "weight": float}
    """
    msg = message.lower()
    best_type = "general"
    best_score = 0
...
def decide_policy(
    ...
    if obs["count"] >= 3 and obs["avg_score"] >= 0.6:
        return {
...
def evaluate_skill_worthiness(
    ...
    # 2. Stability: Are the scores consistent? (low variance = stable)
    ...
    promote = total >= SKILL_PROMOTION_THRESHOLD
    if promote:
        reason = f"Score {total:.2f} >= {SKILL_PROMOTION_THRESHOLD} — promoting to skill"
    else:
        # Find weakest criterion
        weakest = min(criteria_scores, key=criteria_scores.get)
        reason = f"Score {total:.2f} < {SKILL_PROMOTION_THRESHOLD} (weakest: {weakest}={criteria_scores[weakest]:.2f})"
...
def _get_task_observations(task_type: str) -> dict:
    """Get aggregated observations for a task type."""
    data = _load_lifecycle_data()
    return data.get("task_observations", {}).get(task_type, {
        "count": 0,
        "avg_score": 0.0,
        "recent_scores": [],
        "error_rate": 0.0,
...
def record_observation(
    ...
    # Keep last 20 scores for variance calculation
    entry["recent_scores"] = (entry.get("recent_scores", []) + [round(score, 3)])[-20:]
...
def run_lifecycle(
    ...
    had_error = score.get("overall", 0) < 0.4 or "ERROR" in str(score.get("flags", []))
    record_observation(
        task_type=task["type"],
...
def get_lifecycle_status() -> str:
    ...
    if obs:
        ...
        for task_type, info in sorted_tasks[:8]:
            ...
            lines.append(f"  {task_type}: {count}x avg={avg:.2f} err={err:.0%}")
    else:
